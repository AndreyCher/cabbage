...
export function ProxyCountrySelect({ value, onChange }: { value?: string | null; onChange: (value: string | null) => void }) {
  ...
  const countries = [...new Map(items.filter((item) => item.country_code).map((item) => [item.country_code!, item.country_name ?? item.country_code!])).entries()]
  return <FormControl size="small" fullWidth><InputLabel>Proxy country</InputLabel><Select label="Proxy country" value={value ?? ''} onChange={(event) => onChange(event.target.value ? String(event.target.value) : null)}><MenuItem value="">No proxy</MenuItem>{countries.map(([code, name]) => <MenuItem key={code} value={code}>{name} ({code}) · {items.filter((item) => item.country_code === code).length} endpoints</MenuItem>)}</Select></FormControl>
}
