...
export function ProxiesPage() {
  ...
  async function save() {
    const payload = { name: form.name, scheme: form.scheme, host: form.host, port: form.port, username: form.username || null, ...(form.password ? { password: form.password } : {}), bypass: form.bypass || null, verify_ssl: form.verify_ssl, expected_country_code: form.expected_country_code || null, ...(selected ? { enabled: form.enabled } : {}), geoip: { enabled: form.geo_enabled, validate_identity: form.geo_validate, fail_on_mismatch: form.geo_fail } }
  ...
  return <>
    ...
    <Card><CardContent sx={{ overflowX: 'auto' }}><Table sx={{ minWidth: 1000 }}><TableHead><TableRow><TableCell>Name</TableCell><TableCell>Endpoint</TableCell><TableCell>Verified location</TableCell><TableCell>Authentication</TableCell><TableCell>GEO policy</TableCell><TableCell>Status</TableCell><TableCell /></TableRow></TableHead><TableBody>{items.map((item) => { const status = statusView(item.check_status); return <TableRow key={item.id}><TableCell><Typography fontWeight={700}>{item.name}</Typography></TableCell><TableCell>{item.scheme}://{item.host}:{item.port}</TableCell><TableCell>{item.country_code ? <Box><Chip size="small" color="primary" variant="outlined" label={`${item.country_name ?? item.country_code} (${item.country_code})`} /><Typography variant="caption" display="block" color="text.secondary">{item.exit_ip} · {item.timezone}</Typography></Box> : <Typography variant="body2" color="text.secondary">No verified location</Typography>}</TableCell><TableCell>{item.username || item.has_password ? item.username || 'Password only' : 'None'}</TableCell><TableCell>{item.geoip.enabled ? item.geoip.fail_on_mismatch ? 'Strict match' : item.geoip.validate_identity ? 'Validate / warn' : 'Lookup only' : 'Disabled'}</TableCell><TableCell><Stack gap={0.5} alignItems="flex-start"><Chip size="small" label={status.label} color={status.color} variant="outlined" />{item.check_error && <Typography variant="caption" color="error" sx={{ maxWidth: 280, overflowWrap: 'anywhere' }}>{item.check_error}</Typography>}{item.last_checked_at && <Typography variant="caption" color="text.secondary">Checked {new Date(item.last_checked_at).toLocaleString()}</Typography>}</Stack></TableCell><TableCell align="right"><Button startIcon={<VerifiedRounded />} onClick={() => void verify(item)}>Verify</Button><Button startIcon={<EditRounded />} onClick={() => edit(item)}>Edit</Button><Button color="error" startIcon={<DeleteOutlineRounded />} onClick={() => void remove(item)}>Disable</Button></TableCell></TableRow>})}{!items.length && <TableRow><TableCell colSpan={7}><Typography textAlign="center" color="text.secondary" py={4}>No active proxy configurations.</Typography></TableCell></TableRow>}</TableBody></Table></CardContent></Card>
