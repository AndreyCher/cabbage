...
export function IdentitiesPage() {
  ...
  return <>
    ...
    <Card><CardContent sx={{ overflowX: 'auto' }}><Table size="small"><TableHead><TableRow><TableCell>Status</TableCell><TableCell>Name</TableCell><TableCell>Revision</TableCell><TableCell>Updated</TableCell><TableCell /></TableRow></TableHead><TableBody>
      {pagination.pageItems.map((item) => <TableRow key={item.identity} sx={{ height: 44, '& > td': { py: .5 } }}><TableCell><Chip size="small" label={item.in_use ? 'In use' : 'Available'} color={item.in_use ? 'warning' : 'success'} variant="outlined" /></TableCell><TableCell>{item.identity}</TableCell><TableCell>{item.revision}</TableCell><TableCell>{new Date(item.updated_at).toLocaleString()}</TableCell><TableCell align="right" sx={{ whiteSpace: 'nowrap' }}><Button size="small" startIcon={<EditRounded />} onClick={() => edit(item)}>Open</Button><Button size="small" color="error" disabled={item.in_use} startIcon={<DeleteOutlineRounded />} onClick={() => { setDeleteTarget(item); setDeleteAccountData(false); setError('') }}>Delete</Button></TableCell></TableRow>)}
