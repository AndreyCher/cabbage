...
export function ScenariosPage() {
  ...
  function open(item: Scenario) {
    setSelected(item); setName(item.name); setJson(JSON.stringify(item.definition, null, 2)); setEditorTab('actions'); setError('')
  }
  async function importFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setName(importedName); setJson(JSON.stringify({ ...definition, name: importedName }, null, 2)); setEditorTab('actions'); setError('')
  ...
  return <>
    ...
    <Dialog open={Boolean(selected)} onClose={() => setSelected(null)} fullWidth maxWidth="md"><DialogTitle>{selected?.version ? `Edit ${name} v${selected.version}` : 'Import scenario'}</DialogTitle><DialogContent><Stack gap={2} mt={1}>
      ...
      <Tabs value={editorTab} onChange={(_, next) => setEditorTab(next)} variant="scrollable" scrollButtons="auto"><Tab value="actions" label="Scenario actions" /><Tab value="external" label={`External requests (${externalActions.length})`} /></Tabs>
