...
export function WorkersPage() {
  ...
  const [form, setForm] = useState({ identity: 'test-user-001', scenario: '', priority: 0, debug: false, recording: true, proxy_mode: 'default', proxy_config_id: '', timeout_seconds: 3600, worker_config: {} as JsonObject })
  ...
  return <>
    ...
    <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="md"><DialogTitle>Create run</DialogTitle><DialogContent><Stack gap={2} mt={1}>
      ...
      <FormControl><InputLabel>Proxy</InputLabel><Select label="Proxy" value={form.proxy_mode} onChange={(e) => setForm({ ...form, proxy_mode: e.target.value })}><MenuItem value="default">Identity default</MenuItem><MenuItem value="disabled">Disabled</MenuItem><MenuItem value="selected">Select configuration</MenuItem></Select></FormControl>
