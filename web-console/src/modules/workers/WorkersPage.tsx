...
import {
  ...
  Accordion, AccordionDetails, AccordionSummary, IconButton, MenuItem, Select, Stack, Switch, Table, TableBody, TableCell, TableHead,
  TableRow, TableSortLabel, TextField, Tooltip, Typography,
} from '@mui/material'
import { AddRounded, BugReportRounded, DescriptionRounded, ExpandMoreRounded, LiveTvRounded, PlayCircleRounded, RefreshRounded, StopRounded } from '@mui/icons-material'
...
type Run = { id: string; identity: string; scenario_name: string; scenario_version: number; status: string; priority: number; current_stage?: string; created_at: string; debug: boolean; live_stream_available: boolean; recorded_video_available: boolean; logs_available: boolean }
...
const runStatusColor = (status: string) => status === 'completed' ? '#2e7d32'
  : status === 'failed' ? '#d32f2f'
    : status === 'stopped' ? '#757575'
      : status === 'running' ? '#1976d2'
        : status === 'queued' ? '#9e9e9e' : '#ed6c02'
...
export function WorkersPage() {
  ...
  const [form, setForm] = useState({ identity: 'test-user-001', scenario: '', priority: 0, debug: false, recording: true, ignore_identity_location: false, ignore_proxy_requirement: false, proxy_mode: 'default', proxy_config_id: '', timeout_seconds: 3600, worker_config: {} as JsonObject })
  ...
  return <>
    ...
    <Card><CardContent sx={{ overflowX: 'auto' }}><Table size="small" sx={{ minWidth: 900, width: '100%' }}><TableHead><TableRow><TableCell sx={{ width: '1%', whiteSpace: 'nowrap' }}>{sortHeader('status', 'Status')}</TableCell><TableCell sx={{ width: '1%', whiteSpace: 'nowrap' }}>{sortHeader('priority', 'Priority')}</TableCell><TableCell sx={{ width: '1%', whiteSpace: 'nowrap' }}>{sortHeader('scenario', 'Scenario')}</TableCell><TableCell sx={{ width: '1%', whiteSpace: 'nowrap' }}>{sortHeader('stage', 'Stage')}</TableCell><TableCell sx={{ width: 'auto', whiteSpace: 'nowrap' }}>{sortHeader('identity', 'Identity')}</TableCell><TableCell sx={{ width: '1%', whiteSpace: 'nowrap' }}>{sortHeader('created', 'Created')}</TableCell><TableCell align="center" sx={{ width: '1%', whiteSpace: 'nowrap' }}>Video</TableCell><TableCell align="center" sx={{ width: '1%', whiteSpace: 'nowrap' }}>Logs</TableCell></TableRow></TableHead><TableBody>
      {pagination.pageItems.map((run) => { const createdAt = new Date(run.created_at); return <TableRow key={run.id} sx={{ height: 44, '& > td': { py: .5 } }}><TableCell sx={{ width: '1%', whiteSpace: 'nowrap' }}><Stack direction="row" alignItems="center" gap={.5}><Tooltip title={run.status} enterDelay={400}><Box role="img" aria-label={run.status} sx={{ width: 12, height: 12, borderRadius: '50%', bgcolor: runStatusColor(run.status), boxShadow: '0 0 0 2px rgba(0,0,0,.08)', flex: '0 0 auto' }} /></Tooltip>{stoppable.has(run.status) && <Tooltip title="Stop" enterDelay={600}><IconButton color="error" size="small" aria-label="Stop run" onClick={() => void stop(run)}><StopRounded fontSize="small" /></IconButton></Tooltip>}</Stack></TableCell><TableCell sx={{ width: '1%', whiteSpace: 'nowrap' }}>{run.status === 'queued' ? <TextField size="small" type="number" value={priorityDrafts[run.id] ?? String(run.priority)} inputProps={{ min: -100, max: 100, step: 1, 'aria-label': `Priority for ${run.identity}` }} onChange={(event) => setPriorityDrafts((current) => ({ ...current, [run.id]: event.target.value }))} onBlur={() => void changePriority(run)} onKeyDown={(event) => { if (event.key === 'Enter') event.currentTarget.querySelector('input')?.blur() }} sx={{ width: 72, '& .MuiInputBase-root': { height: 30 }, '& .MuiInputBase-input': { px: 1, py: 0 } }} /> : run.priority}</TableCell><TableCell sx={{ width: '1%', whiteSpace: 'nowrap' }}><Stack direction="row" alignItems="center" gap={.5}>{run.debug && <Tooltip title="Debug run" enterDelay={600}><BugReportRounded color="warning" sx={{ fontSize: 17 }} /></Tooltip>}<span>{run.scenario_name}:{run.scenario_version}</span></Stack></TableCell><TableCell sx={{ width: '1%', whiteSpace: 'nowrap' }}>{run.current_stage ?? '—'}</TableCell><TableCell sx={{ width: 'auto', whiteSpace: 'nowrap' }}>{run.identity}</TableCell><TableCell title={createdAt.toISOString()} sx={{ width: '1%', whiteSpace: 'nowrap' }}>{createdTimeFormatter.format(createdAt)}</TableCell><TableCell align="center" sx={{ width: '1%', whiteSpace: 'nowrap' }}>{run.live_stream_available && <Tooltip title="Live stream" enterDelay={600}><IconButton size="small" aria-label="Open live stream" onClick={() => void showMedia(run)}><LiveTvRounded fontSize="small" /></IconButton></Tooltip>}{!run.live_stream_available && run.recorded_video_available && <Tooltip title="Recorded video" enterDelay={600}><IconButton size="small" aria-label="Open recorded video" onClick={() => void showMedia(run)}><PlayCircleRounded fontSize="small" /></IconButton></Tooltip>}{!run.live_stream_available && !run.recorded_video_available && <IconButton size="small" aria-label="Video unavailable" disabled><PlayCircleRounded fontSize="small" /></IconButton>}</TableCell><TableCell align="center" sx={{ width: '1%', whiteSpace: 'nowrap' }}><Tooltip title={run.logs_available ? "Logs" : "No logs available"} enterDelay={600}><span><IconButton size="small" aria-label={run.logs_available ? "Open run logs" : "Logs unavailable"} disabled={!run.logs_available} onClick={() => void showLogs(run)}><DescriptionRounded fontSize="small" /></IconButton></span></Tooltip></TableCell></TableRow> })}
      {!runs.length && <TableRow><TableCell colSpan={8}><Typography color="text.secondary" textAlign="center" py={4}>No runs yet.</Typography></TableCell></TableRow>}
    </TableBody></Table></CardContent><ClientTablePagination count={runs.length} {...pagination} /></Card>
    <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="md"><DialogTitle>Create run</DialogTitle><DialogContent><Stack gap={2} mt={1}>
      ...
      <FormControlLabel control={<Switch disabled={form.debug} checked={form.debug || form.ignore_identity_location} onChange={(e) => setForm({ ...form, ignore_identity_location: e.target.checked })} />} label="Ignore Identity location" />
      <FormControlLabel control={<Switch disabled={form.debug} checked={form.debug || form.ignore_proxy_requirement} onChange={(e) => setForm({ ...form, ignore_proxy_requirement: e.target.checked })} />} label="Ignore proxy requirement" />
      {(form.debug || form.ignore_identity_location || form.ignore_proxy_requirement) && <Alert severity="warning">{form.debug ? 'Debug mode automatically disables proxy use and GEO validation.' : `${form.ignore_identity_location ? 'Identity GEO validation is disabled. ' : ''}${form.ignore_proxy_requirement ? 'This run will start without a proxy. ' : ''}`}The persistent locale, language, timezone and fingerprint remain unchanged.</Alert>}
      <FormControl disabled={form.debug || form.ignore_proxy_requirement}><InputLabel>Proxy</InputLabel><Select label="Proxy" value={form.proxy_mode} onChange={(e) => setForm({ ...form, proxy_mode: e.target.value })}><MenuItem value="default">Identity country pool</MenuItem><MenuItem value="selected">Select configuration</MenuItem></Select></FormControl>
      {!form.debug && !form.ignore_proxy_requirement && form.proxy_mode === 'selected' && <FormControl><InputLabel>Proxy configuration</InputLabel><Select label="Proxy configuration" value={form.proxy_config_id} onChange={(e) => setForm({ ...form, proxy_config_id: e.target.value })}>{proxies.map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</Select></FormControl>}
      <FormControlLabel control={<Switch checked={form.debug} onChange={(e) => setForm({ ...form, debug: e.target.checked })} />} label="Debug mode" />
      ...
      <Accordion disableGutters><AccordionSummary expandIcon={<ExpandMoreRounded />}><Box><Typography fontWeight={700}>Advanced worker overrides</Typography><Typography variant="caption" color="text.secondary">Optional per-run settings, merged over global and Identity configuration.</Typography></Box></AccordionSummary><AccordionDetails><WorkerConfigEditor compact value={form.worker_config} onChange={(worker_config) => setForm({ ...form, worker_config })} /></AccordionDetails></Accordion>
    </Stack></DialogContent><DialogActions><Button onClick={() => setOpen(false)}>Cancel</Button><Button variant="contained" onClick={() => void createRun()} disabled={!form.identity || !form.scenario || (!form.debug && !form.ignore_proxy_requirement && form.proxy_mode === 'selected' && !form.proxy_config_id)}>Create</Button></DialogActions></Dialog>
    ...
    <Dialog open={Boolean(mediaDialog)} onClose={() => setMediaDialog(null)} maxWidth={false} slotProps={{ paper: { sx: { width: mediaWidth, maxWidth: 'calc(100vw - 16px)', maxHeight: 'calc(100vh - 16px)', m: 1, overflow: 'hidden', transition: 'width 160ms ease' } } }}><DialogTitle>{mediaDialog?.live ? 'Live stream' : 'Recorded video'} — {mediaDialog?.run.identity}</DialogTitle><DialogContent sx={{ p: 1, bgcolor: '#05070a', overflow: 'hidden' }}>
      ...
      {!mediaDialog?.live && selectedVideo && <Stack gap={1} sx={{ overflow: 'hidden' }}>
        ...
        <Box key={selectedVideo.name} component="video" controls autoPlay muted playsInline preload="auto" onLoadedMetadata={(event) => { if (event.currentTarget.videoWidth && event.currentTarget.videoHeight) setMediaAspectRatio(event.currentTarget.videoWidth / event.currentTarget.videoHeight) }} src={`${apiRoot}/runs/${mediaDialog.run.id}/videos/${encodeURIComponent(selectedVideo.name)}?ticket=${encodeURIComponent(mediaDialog.ticket)}`} sx={{ width: '100%', maxHeight: mediaDialog.videos.length > 1 ? 'calc(100vh - 201px)' : 'calc(100vh - 152px)', objectFit: 'contain', bgcolor: '#000', display: 'block' }} />
      </Stack>}
