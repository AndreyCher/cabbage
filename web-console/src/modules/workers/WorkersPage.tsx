...
export function WorkersPage() {
  ...
  const [form, setForm] = useState({ identity: 'test-user-001', scenario: '', priority: 0, debug: false, recording: true, ignore_identity_location_and_proxy: false, proxy_mode: 'default', proxy_config_id: '', timeout_seconds: 3600, worker_config: {} as JsonObject })
  ...
  return <>
    ...
    <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="md"><DialogTitle>Create run</DialogTitle><DialogContent><Stack gap={2} mt={1}>
      ...
      <FormControlLabel control={<Switch checked={form.ignore_identity_location_and_proxy} onChange={(e) => setForm({ ...form, ignore_identity_location_and_proxy: e.target.checked })} />} label="Ignore Identity location and proxy requirement" />
      {form.ignore_identity_location_and_proxy && <Alert severity="warning">This run will start without a proxy or GEO validation. The Identity's persistent locale, language, timezone and fingerprint remain unchanged.</Alert>}
      <FormControl disabled={form.ignore_identity_location_and_proxy}><InputLabel>Proxy</InputLabel><Select label="Proxy" value={form.proxy_mode} onChange={(e) => setForm({ ...form, proxy_mode: e.target.value })}><MenuItem value="default">Identity country pool</MenuItem><MenuItem value="selected">Select configuration</MenuItem></Select></FormControl>
      {!form.ignore_identity_location_and_proxy && form.proxy_mode === 'selected' && <FormControl><InputLabel>Proxy configuration</InputLabel><Select label="Proxy configuration" value={form.proxy_config_id} onChange={(e) => setForm({ ...form, proxy_config_id: e.target.value })}>{proxies.map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</Select></FormControl>}
    ...
    </Stack></DialogContent><DialogActions><Button onClick={() => setOpen(false)}>Cancel</Button><Button variant="contained" onClick={() => void createRun()} disabled={!form.identity || !form.scenario || (!form.ignore_identity_location_and_proxy && form.proxy_mode === 'selected' && !form.proxy_config_id)}>Create</Button></DialogActions></Dialog>
