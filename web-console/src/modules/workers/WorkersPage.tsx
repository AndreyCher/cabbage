...
export function WorkersPage() {
  ...
  const refresh = useCallback(async () => {
    try {
      const [nextRuns, nextScenarios, nextProxies, nextIdentities] = await Promise.all([api<Run[]>('/runs'), api<Scenario[]>('/scenarios'), api<Proxy[]>('/proxies'), api<Identity[]>('/identities')])
