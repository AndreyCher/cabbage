...
- global worker defaults grouped into Recording, Diagnostics, Plugins and Advanced JSON; Identity defaults and profiles expose Browser, Fingerprint and Advanced JSON; per-run overrides retain the complete editor;
...
- asynchronous proxy checks with concise pending/healthy/unhealthy table states, automatic refresh and manual recovery for unhealthy endpoints; diagnostic errors remain available through Controller API/DB, while Settings includes monitoring, retries, provider switches and reached-provider quota usage;
...
- consistent non-dynamic pagination for every table, defaulting to 50 items with 25/50/100/150 page-size choices;
- browser-persisted page-size preferences maintained independently for each table;
- full-width table pages with 12-pixel content-edge spacing;
