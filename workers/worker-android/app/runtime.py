...
class RuntimeContext:
    ...
    def put_input(self, key: str, value: Any) -> tuple[bool, str]:
        with self._condition:
            ...
            validator = self.input_validators.get(key)
            if validator:
                try:
                    value = validator(value)
                except (ValueError, FatalActionError):
    ...
    def wait_for_input(self, key: str, timeout_sec: float) -> tuple[bool, Any | None]:
        ...
        source = self.input_sources.pop(key, None)
        if source and not self.has_input(key):
            with self._condition:
                self.status = "waiting_input"
                self.waiting_input = {"key": key, "timeout_sec": timeout_sec,
                                      "started_monotonic": time.monotonic(), "deadline_monotonic": deadline}
                self._event("waiting_input", key=key, timeout_sec=timeout_sec)
            try:
                source(timeout_sec)
            finally:
                self.set_status("running")
            if not self.has_input(key):
                return False, None
        with self._condition:
            if self._shutdown_event.is_set():
                raise ShutdownRequested(self.shutdown_signal)
